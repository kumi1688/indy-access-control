...
function makeSchema(){   
    return new Promise(async (resolve, reject)=>{
        log('스키마 생성...')
        const schemaData = ['full_name', 'issue_date', 'role']
        const [_schemaId, _schema] = await indy.issuerCreateSchema(trustAnchorDid, 'ID_Schema_Lab', '1.0', schemaData)
        schemaId = _schemaId
        schema = _schema
...
function makeCredential(){
    return new Promise(async (resolve, reject)=>{
        ...
        log('CredDef 생성...')
        const [_credDefId, _credDef] = await indy.issuerCreateAndStoreCredentialDef(walletHandle, trustAnchorDid, schema, '연구실 발행', 'CL', {supportRevocation: false})
