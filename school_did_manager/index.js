...
const didManager = require('./school_did_manager.js')
...
app.get('/make_schema', async (req,res)=>{
    ...
    const data = {
        msg: '대학교 학생증 Schema 생성이 완료되었습니다',
        schemaId: id,
        schema: schema
    }
    res.json(data)
...
app.post('/make_cred', async(req,res)=>{
    // const {credReq, credOffer} = req.body    
...
app.get('/make_credential', async(req, res)=>{
    ...
    const data = {
        msg: '대학교 학생증 Credential definition 생성에 성공했습니다',
        credDefId: credDefId,
        credDef: credDef
    }
    res.json(data)
